...
class Canvas():
    ...
    def draw_dashboard(self, frame, gesture = "HOVER", data = {}):
        ...
        # add clear_button
        clear_button_width = int(frame_width *.2) # clear button having 20%
        ...
        if gesture == "ERASE":
            ...
            if 'mid_fing_tip' in data:#changed code due to error
                _, mid_r, mid_c = data['mid_fing_tip']
                # proceed with using mid_r and mid_c
            else:
                # Handle the case where mid_fing_tip is missing
                mid_r, mid_c = None, None
            # Optionally skip drawing or use default values

            # put circle on the map, make it blurr
            img = frame.copy()
