...
class Canvas():
    ...
    def __init__(self, columns, rows):
        ...
        self.color = "GREEN" # only  used to initialize lines
    ...
    def draw_dashboard(self, frame, gesture = "HOVER", data = {}):
        ...
        # add clear_button
        clear_button_width = 180 #int(frame_width *.05) # clear button having 20%
        clear_button_height = 30 #int(frame_height * .1)
        width_border = int(clear_button_width * .03) 
        height_border = int(clear_button_height * .02)
        ...
        button_width = 70 #int((current_width / len(self.colors)) * 0.6)
        button_height = 40 #int(clear_button_height * 0.6)
        width_border = int(button_width * .1)
        height_border = int(button_height *.1)
        ...
        for name_color, color_arr in self.colors.items():
            ...
            frame = cv.rectangle(frame, 
                                (x_dist + width_border, height_border + 5), 
                                (x_dist + button_width - width_border, button_height - height_border + 5),
                                color_arr, 
                                -1)
        ...
        text = f"Mode: {gesture}"
        font_scale = 1.2  # smaller than 2
        thickness = 2

        # Get text size to calculate center
        (text_width, text_height), _ = cv.getTextSize(text, cv.FONT_HERSHEY_SIMPLEX, font_scale, thickness)

        # Calculate position to center it horizontally and place slightly below center
        x_pos = (frame.shape[1] - text_width) // 2
        y_pos = (frame.shape[0] // 2) - 100  # adjust 50 to move lower or higher as needed

        cv.putText(frame, text, (x_pos, y_pos), 
                cv.FONT_HERSHEY_SIMPLEX, 
                font_scale, self.colors[self.color], 
                thickness, cv.LINE_AA)

        # draw the ring if we're in the eraser mode
        if gesture == "ERASE":
            distance = data.get('radius', 0)
            mid_tip = data.get('mid_fing_tip')

            if mid_tip is not None:
                _, mid_r, mid_c = mid_tip
                img = frame.copy()
                cv.circle(img, (mid_r, mid_c), int(distance * 0.5), (0, 255, 255), -1)
                alpha = 0.4
                frame = cv.addWeighted(frame, alpha, img, 1 - alpha, 0)
        elif gesture == "MOVE":
            distance = data.get('radius', 0)  # Default value to avoid KeyError
